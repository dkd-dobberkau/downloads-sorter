...
def main() -> int:
    ...
    parser.add_argument(
        "-v", "--version", action="version", version=f"downloads_sorter {__version__}"
    )

    parser.add_argument(
        "-d", "--directory", type=str, help="Custom downloads directory path"
    )
    ...
    if args.stats:
        ...
        for folder, count in sorted(
            stats["folders"].items(), key=lambda x: x[1], reverse=True
        ):
